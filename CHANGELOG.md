...
- **Nested Groups Documentation Syntax** (#ecf7da2)
  - Fixed incorrect CLI syntax in all nested capture group examples
  - Changed from `'(h.(ll))' red,blue` to `'(h.(ll))' red blue`
  - Root cause: Comma combines colors on SAME group, space separates colors for DIFFERENT groups
  - When using comma: All groups received same color in layers, causing overwrites
  - When using space: Each group gets its own color correctly
  - Fixed examples in: README.md (4 locations), NESTING_DOCS.md (5 locations), CLI help text, test scripts
  - Added clarification: Use commas to combine multiple colors on one group (e.g., `'ERROR' red,bold`)
  - The nested color feature was always working correctly - only documentation showed wrong syntax
...
[2.1.1]: https://github.com/jim-my/pipetint/compare/v2.1.0...v2.1.1
